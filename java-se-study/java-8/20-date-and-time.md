...
`TemporalAccessor` Defines read only access to a temporal object such as date time offset or some combination of these. It
is the super interface of Temporal and by proxy the super interface for the other date and time based structures in the
java time package

Temporal access-or itself directly has no implementations, the actual concrete implementations are based on the child
interface of it - Temporal.

## Temporal

Temporal is the child interface of `TemporalAccessor` and it aims to represent things that are some form of tangible
representation of time concepts from the real world, like dates, time, duration, etc
...
```plaintext
         ...
         │                         │                           │
┌────────┴─────────┐      ┌────────┴─────────┐      ┌──────────┴─────────┐
│ ChronoLocalDate  │      │    LocalTime     │      │ (other Temporals)  │
│(java.time.chrono)│      │  (time-of-day)   │      │ e.g. LocalDateTime │
└────────▲─────────┘      └────────▲─────────┘      └────────────────────┘
         │ implements              │ implements
┌────────┴─────────┐      ┌────────┴─────────┐
│     LocalDate    │      │  (final class)   │
│    (date-only)   │      └──────────────────┘
└──────────────────┘
                   ...
                   │                  │                  │
              ┌────┴─────┐       ┌────┴─────┐      ┌─────┴───────────┐
              │ LocalDate│       │ LocalTime│      │ ChronoLocalDate │
              └──────────┘       └──────────┘      └─────────────────┘
```
...
| minus(Period period)                                            | Returns a `LocalDate` that is the result of subtracting a `Period` from this `LocalDate`. |
| plus(Period period)                                             | Returns a `LocalDate` that is the result of adding a `Period` to this `LocalDate`.        |
| ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ | ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~     |
...
is that based on the number of repetitions of the letter in the pattern that is how a certain element will be
...
to any specific point on the time-line. Rather the value represents a relative amount of time, not an absolute amount of time like the concrete implementations - `LocalDate` or `LocalDateTime`.

The various implementations of this interface are listed below.
...
One of the primary implementations of this interface is the enumeration `ChronoUnit`, yes that is the default
implementation for this interface that aims at overriding the core methods exposed by the `TemporalUnit` interface.
Remember that `ENUMS` are basically constant compile time instances of a class (in this case instances of `ChronoUnit` that
are pre-defined in the code base)

### ChronoUnit

These are the units that `chrono` unit declares internally these units represent. The primary method that is used to
obtain a value from this unit is `getDuration`. Note that the durations here encompass ranges from micro seconds, up to
years and more. How is Duration that is mean to represent only relative time able to do this ? Well exactly that is the
point Duration represents relative time, you can see that even years, months and days are all represented as their
second counter parts.
