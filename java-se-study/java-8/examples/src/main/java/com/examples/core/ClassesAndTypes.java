...
import com.examples.misc.AbstractClassType;
import com.examples.misc.EncapsulatedJavaBean;
import com.examples.misc.FinalClassType;
import com.examples.misc.FinalQualifiedMember;
import com.examples.misc.FinalizeAndDestructors;
import com.examples.misc.NestedClassTypes;
import com.examples.misc.NestedClassTypes.NestedNonStaticClassType;
import com.examples.misc.NestedClassTypes.NestedStaticClassType;
import com.examples.misc.OverrideObjectMethods;
import com.examples.misc.PolymorphicFigureType;
import com.examples.misc.PrivateConstructorClass;
import com.examples.misc.StaticAccessMember;

public class ClassesAndTypes {

    public static final class SuperConstructorClass extends EncapsulatedJavaBean {
        private int o;
        ...
        public SuperConstructorClass(int k) {
            super(null, k);
        }

        public SuperConstructorClass(double m) {
            this((int) m);
            this.o = (int) m;
        }

        public SuperConstructorClass(byte x) {
            // auto-promotion would take effect here evaluating the expression
            super(null, x);
        }

        public SuperConstructorClass(float w) {
            // invalid we reference this before it is constructed
            super(o);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        // showcase how a different permutation of super and this uses in the class constructor can be applie to leverage
        // re-usability
        SuperConstructorClass superConstructorClass = new SuperConstructorClass();
        ...
        FinalQualifiedMember finalQualifiedMember2 = new FinalQualifiedMember() {
            @Override
            // this usage is invad, will produce compile time erorr since we are overriding a declared final method from that class definition
            public int concreteMethod() {
                return 1024;
            }
        };

        // demonstrate that a final class can not be sub-classed, and will be a compile time error, by using an annonymous class
        // here we are effectively creating an annonymous child without a name, which is the same as trying to extend off of
        // this FinalClassType
        ...
        NestedStaticClassType nestedStatic3 = nestedClassTypes.new NestedStaticClassType(null);
