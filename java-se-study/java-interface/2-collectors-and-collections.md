...
- the functional interface has exactly one single abstract non final public method, (SAM) lambda references target
  call these.
- many have default combiners and chainers - andThen, compose, and, or, negate etc.
...
- none of these declare any checked exceptions which is expected, handle exceptions with care
- the lambda method names for the primitive versions of the interfaces in the java.util.function package follow the
  ...
  primitive those can be Int, Double, Long etc. For example applyAsInt, applyAsLong.
